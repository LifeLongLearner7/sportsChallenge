...
export default async function TopPredictorStream() {
  ...
  return (
    <div className="p-6 glass-panel rounded-2xl flex flex-col gap-6 group hover:bg-white/5 transition-all border-white/5 hover:border-primary/30 shadow-xl relative overflow-hidden h-full">
       <div className="absolute top-0 right-0 w-24 h-24 bg-primary/5 blur-3xl -mr-12 -mt-12 pointer-events-none group-hover:bg-primary/10 transition-colors"></div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="pt-2 bg-black/40 rounded-xl p-4 border border-white/5 shadow-inner flex flex-col gap-3">
          <div className="flex justify-between items-center text-[8px] font-black text-primary/60 uppercase tracking-[0.4em] leading-none">
            <span>Tactical Win Rate</span>
            <span className="text-white italic text-xs">{topPredictor?.accuracy || 0}%</span>
          </div>
          <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden p-[1px] relative">
            <div 
              className="h-full bg-gradient-to-r from-primary/40 via-primary to-primary-dim rounded-full shadow-[0_0_10px_rgba(129,236,255,0.4)] transition-all duration-1000"
