...
export default function SettingsClient({ profile }: SettingsClientProps) {
  ...
  const checkComplexity = (pw: string) => ({
    minLength: pw.length >= 6,
    hasUppercase: /[A-Z]/.test(pw),
    hasNumberOrSymbol: /[0-9!@#$%^&*()_+\-=[\]{};':"\\|,.<>\/?]/.test(pw),
  });
  const complexity = checkComplexity(newPassword);
  ...
  const handlePasswordReset = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!passwordOk) {
      setResetError("Security key must be 6+ characters with 1 uppercase and 1 number or symbol.");
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {isOnboarding && (
        <div className="glass-panel p-6 rounded-3xl border-primary/30 bg-primary/5 flex flex-col md:flex-row items-center gap-6 animate-in slide-in-from-top-4 duration-700 shadow-[0_0_40px_rgba(0,229,255,0.08)]">
           <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center text-primary border border-primary/30 shrink-0">
             <Fingerprint size={32} className="animate-pulse" />
           </div>
           <div className="flex-1 text-center md:text-left">
              <h3 className="text-primary font-headline font-black uppercase italic tracking-wider">Choose Your Arena Identity</h3>
              <p className="text-slate-400 text-sm font-bold mt-1 leading-relaxed">
                Pick a unique <span className="text-primary">screen name</span> — this is how other Strategists will know you across the Arena.
              </p>
           </div>
           <div className="px-4 py-2 bg-primary/10 border border-primary/30 rounded-full text-[10px] font-black text-primary uppercase italic tracking-widest hidden lg:block">
              Setup Required
           </div>
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <div className="flex flex-col gap-8">
          {/* Identity Core — shown during onboarding AND normal use */}
                ...
                <div className="flex flex-col gap-3">
                    ...
                    className={cn(
                      ...
                      isSaving 
                        ...
                        : saveSuccess 
                          ...
                          : !screenName.trim()
                            ? "bg-white/[0.03] border border-white/5 text-slate-700 cursor-not-allowed"
                  ...
                  >
                    {isSaving ? (
                      ...
                      <>{isOnboarding ? "IDENTITY CONFIRMED — ENTERING ARENA" : "IDENTITY UPDATED"}</>
                    ) : (
                      <>
                        <Save size={18} />
                        {isOnboarding ? "Confirm Identity & Enter Arena" : "Commit Changes"}
                      </>
                    )}
                  </button>
          ...
          {/* Security Protocol — hidden during onboarding, shown after */}
          {!isOnboarding && (<section className="glass-panel p-8 rounded-3xl border-secondary/20 flex flex-col gap-8 border-white/5 h-fit shadow-xl">
            <div className="flex items-center gap-3">
            ...
            <form onSubmit={handlePasswordReset} className="flex flex-col gap-4">
                    ...
                    {[
                      ...
                      <div key={label} className="flex items-center gap-1.5">
                        <div className={cn(
                          ...
                          ok ? "bg-secondary shadow-[0_0_4px_rgba(255,107,152,0.8)]" : "bg-white/10"
                        )} />
                        <span className={cn(
                          "text-[8px] font-bold uppercase tracking-wider transition-colors",
                          ok ? "text-secondary" : "text-slate-700"
                        )}>{label}</span>
                      </div>
                    ))}
                  </div>
              ...
              <div className="flex flex-col gap-3 mt-2">
                <button 
                  type="submit"
                  disabled={isResetting || !passwordOk}
                  className={cn(
