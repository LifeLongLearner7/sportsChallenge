...
export default function ProfileLayoutClient({
  ...
  return (
    <main className="min-h-screen bg-background pt-32 pb-12 px-6">
      ...
      <div className={cn(
        "max-w-6xl mx-auto flex flex-col md:flex-row gap-8 relative z-10 transition-all",
        isLockdown && "justify-center"
      )}>
        {/* Profile Side Navigation - Locked during onboarding */}
        {!isLockdown && (
          <aside className="w-full md:w-64 flex flex-col gap-4 animate-in fade-in slide-in-from-left-4 duration-500">
        ...
        {/* Content Area - Expanded during lockdown */}
        <div className={cn("flex-1", isLockdown && "max-w-3xl")}>
          {children}
        </div>
      </div>
    </main>
